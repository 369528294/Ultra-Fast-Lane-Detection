...
|------|------|
| 绿点 | 检测到的车道线 |
| 红线 | **目标车道**中心线（当前/左侧/右侧由按键切换） |
| 大红点 | 最底部中心点（用于 error 与 steer） |
| 蓝线 | 画面正中央（视觉中心参考） |
| 左下角文字 | 偏左/居中/偏右 + **滤波后** error；第二行为 `steer: 数值 (10Hz)` 与 **车道: 当前 \| 左侧(并线左) \| 右侧(并线右)** |

### 7.4 并线控制（按键）

前方有障碍物时可手动切换目标车道，红线和 error/steer 会随之切换，用于向左或向右并线：

| 按键 | 作用 |
|------|------|
| **A** 或 **左方向键** | 目标车道改为**左侧车道**（并线左）：红线与 error/steer 按左侧车道中心计算，车会向左侧并线。 |
| **D** 或 **右方向键** | 目标车道改为**右侧车道**（并线右）：红线与 error/steer 按右侧车道中心计算，车会向右侧并线。 |
| **C** 或 **S** | 恢复**当前车道**：红线与 error/steer 回到本车所在车道。 |
| **Q** 或 **Esc** | 退出程序。 |

逻辑说明：所有检测到的车道按平均横坐标从左到右排序，默认取「夹住画面中心」的一对作为当前车道并求中线；按左键时改为取「当前车道左边那一对」的中线，按右键时改为「当前车道右边那一对」的中线，从而在不改控制结构的前提下实现并线目标切换。
...
- **目标车道中心线**：用画面中心两侧最近的两条线求中线得红线；支持**并线**：按 A/左键 或 D/右键 将目标车道切到左侧或右侧，红线和 error/steer 随之切换。
...
- **steer**：PID 在 10Hz 下输出并限幅后的**转向修正量**，接到舵机/差速等即可实现车道居中或并线导航。

上述逻辑均在 `demo_camera.py` 中实现，可直接在该脚本中调整 PID 参数、滤波系数、控制频率、并线按键或接入实际机器人控制接口。
